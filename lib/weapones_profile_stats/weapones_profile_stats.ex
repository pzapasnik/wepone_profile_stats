defmodule WeaponesProfileStats.Application do
  ...
  def start(_type, _args) do
    ...
    weapons =
      case config[:execution] do
        :io_input ->
          ...
          IOWeapons.get_io_input()

        :file_input ->
          CSVWeapons.read(config[:file_path])
    ...
    IO.puts("Result Weapons:")
    IO.inspect(weapons)

    {adapted_weapons, errors} = WeaponAdapter.adapt_weapons(weapons)

    IO.puts("Adapted Weapons:")
    IO.inspect(adapted_weapons)
